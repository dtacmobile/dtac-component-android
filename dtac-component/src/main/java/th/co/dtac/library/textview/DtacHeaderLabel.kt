...
class DtacHeaderLabel : DtacLabel {
    ...
    private fun setupProperties() {
        setTextSize(if (isScale) TypedValue.COMPLEX_UNIT_SP else TypedValue.COMPLEX_UNIT_DIP, 18f)
    ...
    private fun setFont() {
        val font: Typeface? = when (DtacWidget.currentLanguage().toUpperCase()) {
            "MM", "MY" -> {
                ...
                ResourcesCompat.getFont(context, R.font.dtac2018_bold)
            }
        }
        typeface = font ?: ResourcesCompat.getFont(context, R.font.dtac2018_bold)
    }
}